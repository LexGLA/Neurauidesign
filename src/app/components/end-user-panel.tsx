...
// ========================
function EuHeader() {
  const { openModal, euScreen } = useApp();
  ...
  return (
    <header className="flex-shrink-0 border-b border-[var(--aw-border)]" style={{ background: 'var(--aw-bg-header)', backdropFilter: 'blur(20px)' }}>
      <div className="flex items-center px-3 md:px-6 pt-2 mb-1.5 relative">
        ...
        {/* Center: Neura Logo / Desktop: screen title */}
        ...
        {/* Left side: settings (mobile only) */}
...
function EuNav() {
  const { euScreen, setEuScreen, orders } = useApp();
  ...
  const badgeMap: Partial<Record<EuScreen, number>> = {
    euChatListScreen: 3, // unread messages
...
// EU HOME
// ========================
function EuHomeScreen() {
  const { openChat, setEuScreen, euProfile, agents, openModal, orders } = useApp();

  const quickActions = [
    { icon: 'fa-solid fa-utensils', bg: 'aw-bg-teal', title: 'سفارش غذا', desc: 'سفارش از رستوران', action: () => setEuScreen('euDineScreen') },
    { icon: 'fa-solid fa-store', bg: 'aw-bg-amber', title: 'مارکت', desc: 'خرید از فروشگاه‌ها', action: () => setEuScreen('euMarketScreen') },

    { icon: 'fa-solid fa-headset', bg: 'aw-bg-rose', title: 'پشتیبانی', desc: 'رفع مشکلات', action: () => {
      const a = agents.find(x => x.id === 'support');
      if (a && a.locked) { openModal('فعال‌سازی ' + (a.name || 'پشتیبانی'), <SubscribeContent agentId={a.id} />); return; }
      setEuScreen('euSupportScreen');
    }},
  ];
  ...
  const euAgentIds = ['restaurant', 'assistant', 'support', 'market'];
  const euAgents = euAgentIds.map(id => agents.find(a => a.id === id)).filter((a): a is Agent => a != null && !a.locked);
  const lastAgent = [...euAgents].sort((a, b) => {
    const tm: Record<string, number> = { 'الان': 0, '۲ دقیقه': 2, '۵ دقیقه': 5, '۱۰ دقیقه': 10, '۳۰ دقیقه': 30 };
    return (tm[a.lastTime || ''] || 99) - (tm[b.lastTime || ''] || 99);
  })[0];

  const promos = [
    { id: 'p1', icon: 'fa-solid fa-percent', bg: '#EF4444', title: '۳۰٪ تخفیف اولین سفارش', desc: 'تا پایان هفته', action: () => setEuScreen('euDineScreen') },
    { id: 'p2', icon: 'fa-solid fa-gift', bg: '#8B5CF6', title: 'ارسال رایگان مارکت', desc: 'برای خرید بالای ۲۰۰ هزار تومان', action: () => setEuScreen('euMarketScreen') },
  ];

  return (
    <div className="flex-1 overflow-y-auto pb-4 aw-scroll">
      {/* Profile Card */}
      <div className="flex items-center gap-3.5 mx-4 mt-4 p-5 rounded-2xl border relative overflow-hidden" style={{ background: 'linear-gradient(135deg, var(--aw-eu-primary-dark), var(--aw-eu-primary))', borderColor: 'rgba(126,95,170,0.25)' }}>
        <div className="absolute top-[-20px] left-[-20px] w-[80px] h-[80px] rounded-full opacity-10" style={{ background: 'white' }} />
        <div className="absolute bottom-[-30px] right-[-10px] w-[60px] h-[60px] rounded-full opacity-5" style={{ background: 'white' }} />
        <div className="w-[52px] h-[52px] rounded-[14px] flex items-center justify-center text-white text-xl relative" style={{ background: 'rgba(255,255,255,0.15)', fontWeight: 700 }}>
          <i className="fa-solid fa-wallet text-[22px]" />
        </div>
        <div className="flex-1 relative">
          <p className="text-[11px] m-0" style={{ color: 'rgba(255,255,255,0.6)' }}>موجودی کیف پول</p>
          <h3 className="text-[20px] text-white m-0 mt-0.5" style={{ fontWeight: 800, direction: 'ltr', textAlign: 'right' }}>۲,۴۵۰,۰۰۰ <span className="text-[11px]" style={{ fontWeight: 500, color: 'rgba(255,255,255,0.5)' }}>تومان</span></h3>
        </div>
        <button className="w-9 h-9 rounded-xl border bg-transparent text-white cursor-pointer flex items-center justify-center relative transition-all" style={{ borderColor: 'rgba(255,255,255,0.2)', background: 'rgba(255,255,255,0.1)' }}
          onClick={() => openModal('کیف پول', <WalletModal />)}>
      ...
      <div className="px-4 mt-3">
        <div className="flex gap-2 overflow-x-auto pb-1 aw-scroll">
          {promos.map(p => (
            <div key={p.id} className="flex items-center gap-2.5 p-3 rounded-xl cursor-pointer flex-shrink-0 min-w-[220px] transition-all"
              style={{ background: `linear-gradient(135deg, ${p.bg}18, ${p.bg}08)`, border: `1px solid ${p.bg}25` }}
              onClick={p.action}>
              <div className="w-9 h-9 rounded-lg flex items-center justify-center text-white flex-shrink-0" style={{ background: p.bg }}>
                <i className={`${p.icon} text-[13px]`} />
              </div>
              <div className="min-w-0">
                <div className="text-[12px] text-[var(--aw-text-primary)]" style={{ fontWeight: 700 }}>{p.title}</div>
                <div className="text-[10px] text-[var(--aw-text-muted)]">{p.desc}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      <div className="px-4 mt-3">
        <div className="text-[12px] text-[var(--aw-text-muted)] mb-2 px-1" style={{ fontWeight: 700 }}>
          <i className="fa-solid fa-bolt" /> دسترسی سریع
        </div>
        <div className="grid grid-cols-4 md:grid-cols-6 gap-2 mb-3">
          {quickActions.map((item, i) => {
            const agentIds = ['restaurant', 'market', 'assistant', 'support'];
            const lockableIds = ['assistant', 'support'];
            const agentId = agentIds[i];
            const isLocked = lockableIds.includes(agentId) && agents.find(x => x.id === agentId)?.locked;
            return (
              <div key={i}
                className={`flex flex-col items-center gap-2 p-3 rounded-[14px] cursor-pointer border transition-all text-center ${isLocked ? 'opacity-50' : ''}`}
                style={{ background: 'var(--aw-eu-card)', borderColor: 'rgba(126,95,170,0.15)' }}
                onClick={item.action}>
                <div className={`w-11 h-11 rounded-xl flex items-center justify-center text-white text-lg ${item.bg}`}>
                  <i className={item.icon} />
                </div>
                <div className="text-[11px]" style={{ fontWeight: 600 }}>{item.title}</div>
              </div>
            );
          })}
        </div>
      ...
      {activeOrders.length > 0 && (
        <div className="px-4 mt-3">
          <div className="flex items-center justify-between mb-2 px-1">
            <div className="text-[12px] text-[var(--aw-text-muted)]" style={{ fontWeight: 700 }}>
              <i className="fa-solid fa-truck-fast" /> سفارشات من
            </div>
            <button className="text-[11px] text-[var(--aw-eu-primary)] bg-transparent border-none cursor-pointer" style={{ fontWeight: 600 }}
              onClick={() => setEuScreen('euOrdersScreen')}>
              مشاهده همه <i className="fa-solid fa-chevron-left text-[8px] mr-0.5" />
            </button>
          </div>
          {activeOrders.slice(0, 2).map(o => (
            <div key={o.id} className="flex items-center gap-3 p-3 rounded-xl mb-1.5 cursor-pointer border transition-all hover:border-[var(--aw-eu-primary)]"
              style={{ background: 'var(--aw-eu-card)', borderColor: 'rgba(126,95,170,0.15)' }}
              ...
              <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0"
                ...
                <i className={`fa-solid ${o.status === 'preparing' ? 'fa-fire-burner' : 'fa-hourglass-half'} text-[14px]`}
                  style={{ color: ORDER_STATUS_COLORS[o.status]?.text || 'var(--aw-eu-primary)' }} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-[12px]" style={{ fontWeight: 600 }}>سفارش #{o.num}</span>
                  <span className="text-[9px] px-1.5 py-0.5 rounded-md" style={{ background: ORDER_STATUS_COLORS[o.status]?.bg, color: ORDER_STATUS_COLORS[o.status]?.text, fontWeight: 600 }}>
                    {ORDER_STATUS_LABELS[o.status]}
                  </span>
                </div>
                <div className="text-[11px] text-[var(--aw-text-secondary)] truncate">{o.desc}</div>
              </div>
              <span className="text-[11px] text-[var(--aw-text-muted)]">{o.price}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="px-4 mt-3">
        <div className="grid grid-cols-3 gap-2">
          <div className="flex flex-col items-center p-3 rounded-xl border cursor-pointer transition-all hover:border-[var(--aw-eu-primary)]"
            style={{ background: 'var(--aw-eu-card)', borderColor: 'rgba(126,95,170,0.15)' }}
            onClick={() => setEuScreen('euOrdersScreen')}>
            <span className="text-[18px] text-[var(--aw-eu-primary)]" style={{ fontWeight: 800 }}>{toFa(orders.length)}</span>
            <span className="text-[10px] text-[var(--aw-text-muted)]" style={{ fontWeight: 500 }}>کل سفارشات</span>
          </div>
          <div className="flex flex-col items-center p-3 rounded-xl border cursor-pointer transition-all hover:border-[#10B981]"
            style={{ background: 'var(--aw-eu-card)', borderColor: 'rgba(126,95,170,0.15)' }}
            onClick={() => setEuScreen('euOrdersScreen')}>
            <span className="text-[18px] text-[#10B981]" style={{ fontWeight: 800 }}>{toFa(deliveredOrders.length)}</span>
            <span className="text-[10px] text-[var(--aw-text-muted)]" style={{ fontWeight: 500 }}>تحویل شده</span>
          </div>
          <div className="flex flex-col items-center p-3 rounded-xl border cursor-pointer transition-all hover:border-[#F59E0B]"
            style={{ background: 'var(--aw-eu-card)', borderColor: 'rgba(126,95,170,0.15)' }}
            onClick={() => setEuScreen('euChatListScreen')}>
            <span className="text-[18px] text-[#F59E0B]" style={{ fontWeight: 800 }}>{toFa(euAgents.length)}</span>
            <span className="text-[10px] text-[var(--aw-text-muted)]" style={{ fontWeight: 500 }}>ایجنت فعال</span>
          </div>
        </div>
      </div>
...
function EuAppSettings() {
  ...
  const themeOptions: { id: 'dark' | 'light' | 'bw'; icon: string; label: string }[] = [
...
export default function EndUserPanel() {
  ...
  // Agent-specific screens take over full layout (own header + footer)
  ...
  return (
    <div className="flex h-full min-h-0" style={{ direction: 'rtl' }}>
      {/* Desktop Sidebar — RIGHT side (order:-1 ensures rightmost in RTL) */}
      <EuSidebar expanded={sidebarExpanded} onToggle={() => setSidebarExpanded(p => !p)} />
