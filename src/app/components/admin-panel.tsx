...
function AdminHeader() {
  const { company, setCompany, openModal, setAdminScreen, adminScreen, agentTeam } = useApp();

  return (
    <header className="flex-shrink-0 border-b border-[var(--aw-border)]" style={{ background: 'var(--aw-bg-header)', backdropFilter: 'blur(20px)' }}>
      <div className="flex items-center mb-1.5 relative px-[12px] pt-[15px] pb-[5px]">
        {/* Right side (mobile): search + bell + company selector */}
        ...
        <div className="flex-1 flex items-center justify-center">
          <svg className="h-[28px] w-auto md:hidden" fill="none" viewBox="0 0 305 157">
              <path d={svgPaths.p2d0da9f0} fill="url(#neuraHeaderGrad)" />
              <defs>
                <linearGradient gradientUnits="userSpaceOnUse" id="neuraHeaderGrad" x1="-1" x2="308.36" y1="0.05" y2="155.33">
                  <stop stopColor="#26F8FF" />
                  <stop offset="1" stopColor="#1641FF" />
                </linearGradient>
              </defs>
            </svg>
          {agentTeam && (
        ...
        <div className="flex gap-0.5 z-10">
          <div className="md:hidden">
            <HeaderBtn icon="fa-solid fa-cog" onClick={() => openModal('تنظیمات', <MoreScreenModal />)} />
          </div>
      ...
      {/* Team icons: center top of page (both mobile & desktop) */}
      <TeamIconBar />
    </header>
  );
}
...
export function RoleSwitcher() {
  const { role, setRole, setBriefingSeen } = useApp();
  return (
        ...
        <button
          ...
          className={`flex-1 py-1.5 px-2 border-none rounded-lg text-[12px] cursor-pointer transition-all ${
            role === r
              ...
              : 'bg-transparent text-[var(--aw-text-muted)]'
          }`}
...
function AdminNav() {
  const { adminScreen, setAdminScreen, agentTeam } = useApp();
  ...
  return (
    <nav
      className="flex-shrink-0 flex border-t border-[var(--aw-border)] px-2 py-1.5 md:hidden"
      style={{ background: 'var(--aw-bg-header)', backdropFilter: 'blur(20px)', paddingBottom: 'max(6px, env(safe-area-inset-bottom))' }}
    >
      {navItems.map((item) => (
        <button
          key={item.id}
          className={`flex-1 flex flex-col items-center gap-0.5 py-2 px-1 border-none bg-transparent cursor-pointer transition-all rounded-[10px] text-[10px] ${
            activeId === item.id ? 'text-[var(--aw-primary)]' : 'text-[var(--aw-text-muted)]'
          }`}
...
function AdminSidebar({ expanded, onToggle }: { expanded: boolean; onToggle: () => void }) {
  ...
  return (
    ...
    >
      ...
      {/* Company selector */}
      {expanded ? (
        <div className="mx-3 mt-3 mb-2 px-2.5 py-2 rounded-[10px] border border-[var(--aw-border)] cursor-pointer relative" style={{ background: 'var(--aw-bg-input)' }}>
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded-lg flex items-center justify-center text-[var(--aw-primary)] flex-shrink-0" style={{ background: 'var(--aw-primary-bg)' }}>
              <i className="fa-solid fa-building text-xs" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-[9px] text-[var(--aw-text-muted)]">شرکت فعال</div>
              <div className="text-[12px] truncate" style={{ fontWeight: 600 }}>{COMPANIES[company]?.name}</div>
            </div>
            <i className="fa-solid fa-chevron-down text-[10px] text-[var(--aw-text-muted)]" />
          </div>
          <select className="absolute inset-0 opacity-0 cursor-pointer text-base" value={company} onChange={(e) => setCompany(e.target.value)}>
            {Object.entries(COMPANIES).map(([k, v]) => <option key={k} value={k}>{v.name}</option>)}
          </select>
        ...
        <div className="flex justify-center mt-3 mb-2 relative">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center text-[var(--aw-primary)] cursor-pointer" style={{ background: 'var(--aw-primary-bg)' }} title={COMPANIES[company]?.name}>
      ...
      {/* Nav Items */}
      <nav className="flex-1 overflow-y-auto px-2 py-2 aw-scroll">
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full flex items-center gap-3 mb-1 border-none rounded-xl cursor-pointer transition-all text-[13px] ${
        ...
        {(chatTab === 'personnel' || chatTab === 'all') && (() => {
          ...
          return visible.map(c => {
            ...
            return (
              <div key={'pp_' + c.id}
                className="flex items-center gap-3 p-3 rounded-[14px] cursor-pointer mb-1 border border-transparent hover:bg-[var(--aw-bg-card-hover)] transition-all"
                onClick={() => openModal('گفتگوی پرسنل', <PersonnelPeerChatViewer chat={c} personnel={personnel} />)}>
