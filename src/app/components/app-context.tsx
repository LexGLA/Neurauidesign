...
export type ThemeMode = 'dark' | 'light' | 'bw';
...
interface ChatState {
  open: boolean;
  id: string | null;
  type: 'agent' | 'personnel' | 'eu' | 'customer' | 'group' | null;
  topicId: number | null;
  topicsOpen: boolean;
}
...
interface AppContextType {
  ...
  chat: ChatState;
  isTyping: boolean;
  openChat: (id: string, type: 'agent' | 'personnel' | 'eu' | 'customer' | 'group') => void;
  closeChat: () => void;
  toggleTopics: () => void;
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const [theme, setTheme] = useState<ThemeMode>(() => {
    try {
      ...
      if (saved === 'dark' || saved === 'light' || saved === 'bw') return saved;
    } catch {}
    return 'light';
  });
  ...
  // Open chat
  const openChat = useCallback((id: string, type: 'agent' | 'personnel' | 'eu' | 'customer' | 'group') => {
    ...
    setChatState({ open: true, id, type, topicId, topicsOpen: false });
    setIsTyping(false);
  ...
  const getMessages = useCallback((): Message[] => {
    ...
    return [];
  }, [chat, company, topicsMap, pMsgsMap, euMsgsMap, custMsgsMap, groupMsgsMap]);
